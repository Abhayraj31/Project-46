...
var playertank, player1L_img, player1R_img, player1U_img, player1D_img,
...
function draw() {
  ...
  move(playertank);
  playertank.collide(edges);
  if (keyDown("space") && playeralive === true) {
    pbullet(playertank, player_direction);
  }
  ...
  if (ebulletGroup.isTouching(playertank)) {
    image(boomimage, playertank.x, playertank.y);
    playertank.destroy();
    playeralive = false;
    playertanklives--;
...
function spawnPlayerTank() {
  playertank = createSprite(250, 380, 50, 50);
  playertank.addImage(player1U_img);
